...
export default class SunsetScreen extends React.Component {
    ...
    try {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
        {
          ...
          message:
            'React Sunrise Sunset App needs access to your current location ',
        }
      );
  ...
  getPredictions(city) {
    axios
      .get(
        `https://maps.googleapis.com/maps/api/place/autocomplete/json?key=${REACT_APP_API_KEY}&input=${city}`
      )
      .then(response =>
        this.setState({
  ...
  getPlaceLocation(id) {
    axios
      .get(
        `https://maps.googleapis.com/maps/api/place/details/json?key=${REACT_APP_API_KEY}&placeid=${id}`
      )
      .then(response => {
        this.getSunriseInfo(
  ...
  getAddressByLocation(lat, lng) {
    axios
      .get(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${lat},${lng}&key=${REACT_APP_API_KEY}`
      )
      .then(response => {
  ...
  render() {
    ...
    return (
      <View style={styles.root}>
        ...
        />
        <Button title="Next screen" onPress={() => navigate('Counter')} />
        <View style={styles.predictions}>
          <Predictions
            onPredictionPress={this.onPredictionPress.bind(this)}
